...
User Story 100 adds a **Sort: Earliest appointment** option to the DocSeek results page, allowing busy patients to reorder physician recommendations by their next available appointment slot.

**Sorting logic.** The implementation is entirely client-side. `sortDoctorsByEarliestAppointment` partitions the loaded `Doctor[]` into those with a parseable `next_available` datetime and those without. Doctors with a date are sorted ascending by timestamp; when two datetimes are equal, the secondary key is `match_score` descending for determinism. Doctors without a date are appended at the end.

**Persistence.** The selected sort option (`"relevance"` or `"earliest_appointment"`) is written to `sessionStorage` under the key `"docseek-sort-option"` by `saveSortOption` and read back on the next page load by `loadSortOption`. Both functions degrade gracefully when `sessionStorage` is unavailable.

**Data model changes.** The `next_available` field (`TIMESTAMPTZ`, nullable) was added to the `doctors` database table and is now selected in both `querySearchDoctors` and `queryGetDoctorById` (`api/src/queries.ts`). The field is exposed through `DoctorRow` (`api/src/search.ts`) and the client `Doctor` type in `App.tsx`. On the doctor card, `formatNextAvailable` renders the value with a locale-aware month/day/year/hour/minute/timezone format, and falls back to "No appointment data" when the field is absent or unparseable.

**Scope.** No new external dependencies, no new database tables, and no API endpoint changes were required. The story builds entirely on the existing physician-matching infrastructure. Primary ownership is **TBD** with secondary **Yuxiang Huang**; the feature was merged into `main` on **2026-04-19**.
